...
string AL_GetLocateWrapperCustomAnims(int nActivity)
{
    ...
    if (nActivity == 93) return "sitfidget, sitidle, sittalk, sittalk01, sittalk02";
    if (nActivity == 94) return "kneelidle, kneeltalk";
    ...
    if (nActivity == 97) return "meditate";
    if (nActivity == 98) return "disableground, sleightofhand, sneak";
    return "";
}
...
string AL_GetActivityCustomAnims(int nActivity)
{
    ...
    if (nActivity == AL_ACT_NPC_ACT_ONE || nActivity == AL_ACT_NPC_ACT_TWO || nActivity == AL_ACT_NPC_TRAINING_ONE || nActivity == AL_ACT_NPC_TRAINING_TWO || nActivity == AL_ACT_NPC_TRAINER_PACE || nActivity == AL_ACT_NPC_POST)
    {
        return "lookleft, lookright";
    }

    if (nActivity == AL_ACT_NPC_DINNER || nActivity == AL_ACT_NPC_WAKE)
    {
        return "sitdrink, siteat, sitidle";
    }
    ...
    if (nActivity == AL_ACT_NPC_MEDITATE) return "meditate";
    if (nActivity == AL_ACT_NPC_READ) return "sitidle, sitread, sitteat";
    if (nActivity == AL_ACT_NPC_SIT) return "sitfidget, sitidle, sittalk, sittalk01, sittalk02";
    ...
    if (nActivity == AL_ACT_NPC_THIEF2) return "disableground, sleightofhand, sneak";
    ...
    if (nActivity == AL_ACT_NPC_KNEEL_TALK) return "kneelidle, kneeltalk";
    if (nActivity == AL_ACT_NPC_BARMAID || nActivity == AL_ACT_NPC_BARTENDER) return "gettable, lookright, openlock, yawn";
