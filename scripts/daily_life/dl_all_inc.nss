...
object DL_FindAnchorByTag(object oArea, string sTag)
{
    ...
    while (GetIsObjectValid(oObj))
    {
        ...
        if ((nObjType == OBJECT_TYPE_WAYPOINT || nObjType == OBJECT_TYPE_PLACEABLE) && GetTag(oObj) == sTag)
        {
            return oObj;
        }
...
object DL_GetConversationStore(object oNPC)
{
    ...
    if (nAreaMatches > 1)
    {
        DL_LogNpc(oNPC, DL_DEBUG_BASIC, "conversation store tag conflict in area: area_tag=" + GetTag(oNpcArea) + ", store_tag=" + sStoreTag);
        return OBJECT_INVALID;
    }
    if (nAreaMatches == 1)
    {
