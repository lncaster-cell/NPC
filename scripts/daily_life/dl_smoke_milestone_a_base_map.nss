...
void DL_LogReadinessIssue(object oNPC, string sField, int nValue, string sExpected)
{
    DL_Log(
        DL_DEBUG_BASIC,
        "MilestoneA base-map readiness issue npc=" + GetTag(oNPC) +
        " field=" + sField +
...
void DL_LogReadinessIssueStr(string sOwnerTag, string sField, string sValue, string sExpected)
{
    DL_Log(
        DL_DEBUG_BASIC,
        "MilestoneA base-map readiness issue owner=" + sOwnerTag +
        " field=" + sField +
        " value=" + sValue +
        " expected=" + sExpected
    );
...
int DL_BaseMapHasAnyExplicitRoute(object oBase)
{
    if (GetLocalString(oBase, DL_L_BASE_WORK_AREA_TAG) != "") return TRUE;
    if (GetLocalString(oBase, DL_L_BASE_SLEEP_AREA_TAG) != "") return TRUE;
    if (GetLocalString(oBase, DL_L_BASE_SERVICE_AREA_TAG) != "") return TRUE;
    if (GetLocalString(oBase, DL_L_BASE_SOCIAL_AREA_TAG) != "") return TRUE;
    if (GetLocalString(oBase, DL_L_BASE_PUBLIC_AREA_TAG) != "") return TRUE;
    if (GetLocalString(oBase, DL_L_BASE_DUTY_AREA_TAG) != "") return TRUE;
    if (GetLocalString(oBase, DL_L_BASE_GATE_AREA_TAG) != "") return TRUE;
    if (GetLocalString(oBase, DL_L_BASE_HIDE_AREA_TAG) != "") return TRUE;
    return FALSE;
}
...
int DL_RunReadinessChecks()
{
    ...
    while (GetIsObjectValid(oArea))
    {
        ...
        {
            DL_Log(DL_DEBUG_BASIC, "MilestoneA base-map readiness issue area=" + GetTag(oArea) + " tier=" + IntToString(nTier) + " expected=0..2");
            nErrors += 1;
        }
        while (GetIsObjectValid(oObject))
        {
            if (GetObjectType(oObject) == OBJECT_TYPE_CREATURE && !GetIsPC(oObject) && DL_IsDailyLifeNpc(oObject))
            {
                ...
                object oBase = DL_GetNpcBase(oObject);
                string sOwnerTag;
                nNpcCount += 1;
                ...
                {
                    DL_LogReadinessIssueStr(GetTag(oObject), DL_L_NPC_BASE, "<invalid>", "valid base object");
                    nErrors += 1;
                }
                else
                {
                    sOwnerTag = GetTag(oBase);
                    if (GetLocalString(oBase, DL_L_BASE_ID) == "")
                    {
                        DL_LogReadinessIssueStr(sOwnerTag, DL_L_BASE_ID, "", "non-empty base id");
                        nErrors += 1;
                    }
                    if (GetLocalString(oBase, DL_L_BASE_EXTERIOR_AREA_TAG) == "" && GetLocalString(oBase, DL_L_BASE_INTERIOR_AREA_TAG) == "")
                    {
                        DL_LogReadinessIssueStr(sOwnerTag, "dl_base_area_tags", "", "at least one exterior/interior area tag");
                        nErrors += 1;
                    }
                    if (GetLocalString(oBase, DL_L_BASE_ENTRY_EXTERIOR_TAG) == "" && GetLocalString(oBase, DL_L_BASE_ENTRY_INTERIOR_TAG) == "")
                    {
                        DL_LogReadinessIssueStr(sOwnerTag, "dl_base_entry_tags", "", "at least one entry anchor tag");
                        nWarnings += 1;
                    }
                    if (!DL_BaseMapHasAnyExplicitRoute(oBase))
                    {
                        DL_LogReadinessIssueStr(sOwnerTag, "dl_base_routes", "", "at least one explicit route area tag");
                        nWarnings += 1;
                    }
                }
                if (!bNamed && !bPersistent)
                {
                    DL_Log(DL_DEBUG_BASIC, "MilestoneA base-map readiness warning npc=" + GetTag(oObject) + " has no worker guarantee flags (dl_named or dl_persistent)");
                    nWarnings += 1;
                }
            }
    ...
    {
        DL_Log(DL_DEBUG_BASIC, "MilestoneA base-map readiness warning module local dl_smoke_trace=FALSE");
        nWarnings += 1;
    }
    if (nAreaCount == 0)
    {
        DL_Log(DL_DEBUG_BASIC, "MilestoneA base-map readiness issue no areas found in module");
        nErrors += 1;
    }
    if (nHotCount == 0)
    {
        DL_Log(DL_DEBUG_BASIC, "MilestoneA base-map readiness issue no HOT areas found (dl_area_tier=2)");
        nErrors += 1;
    }
    if (nNpcCount == 0)
    {
        DL_Log(DL_DEBUG_BASIC, "MilestoneA base-map readiness issue no Daily Life NPC found");
        nErrors += 1;
    }
    DL_Log(DL_DEBUG_BASIC, "MilestoneA base-map readiness summary areas=" + IntToString(nAreaCount) + " hot_areas=" + IntToString(nHotCount) + " dl_npc=" + IntToString(nNpcCount) + " warnings=" + IntToString(nWarnings) + " errors=" + IntToString(nErrors));
...
void DL_LogScenarioResult(string sScenario, int bFound, int bPass, string sDetail)
{
    ...
    DL_Log(DL_DEBUG_BASIC, "MilestoneA base-map smoke " + sScenario + " status=" + sStatus + " detail=" + sDetail);
...
{
    DL_Log(DL_DEBUG_BASIC, "MilestoneA base-map smoke " + sScenario + " counters checked=" + IntToString(nChecked) + " passed=" + IntToString(nPassed));
}
...
void main()
{
    ...
    {
        DL_Log(DL_DEBUG_BASIC, "MilestoneA base-map smoke overall aborted due to readiness errors=" + IntToString(nReadinessErrors));
        DL_ClearScenarioMarkers();
        return;
    }
