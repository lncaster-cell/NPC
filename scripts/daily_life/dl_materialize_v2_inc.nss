#ifndef DL_MATERIALIZE_V2_INC_NSS
#define DL_MATERIALIZE_V2_INC_NSS
...
#include "dl_anchor_v2_inc"
#include "dl_base_context_inc"
...
object DL_FindConfiguredAnchorPoint(object oNPC, object oTargetArea, int nDirective, int nAnchorGroup)
{
    string sAnchorTag = DL_ResolveBaseContextAnchorTag(oNPC, nDirective, nAnchorGroup);
    ...
    return DL_FindAnchorByTag(oTargetArea, sAnchorTag);
}

object DL_FindConfiguredOrDerivedAnchorPoint(object oNPC, object oTargetArea, int nDirective, int nAnchorGroup)
{
    object oPoint = DL_FindConfiguredAnchorPoint(oNPC, oTargetArea, nDirective, nAnchorGroup);
    ...
    return DL_FindAnchorPoint(oNPC, oTargetArea, nAnchorGroup);
}

object DL_FindConfiguredOrDerivedAnchorPointIgnoringPolicy(object oNPC, object oTargetArea, int nDirective, int nAnchorGroup)
{
    object oPoint = DL_FindConfiguredAnchorPoint(oNPC, oTargetArea, nDirective, nAnchorGroup);
    ...
    return DL_FindAnchorPointIgnoringPolicy(oNPC, oTargetArea, nAnchorGroup);
}
...
void DL_MaterializeNpc(object oNPC, object oArea)
{
    ...
    oPoint = DL_FindConfiguredOrDerivedAnchorPoint(oNPC, oTargetArea, nDirective, nAnchorGroup);

    if (!GetIsObjectValid(oPoint))
    {
        oPolicyFilteredAnchor = DL_FindConfiguredOrDerivedAnchorPointIgnoringPolicy(oNPC, oTargetArea, nDirective, nAnchorGroup);
        if (GetIsObjectValid(oPolicyFilteredAnchor))
        {
            DL_LogNpc(
                oNPC,
                DL_DEBUG_BASIC,
                "anchor filtered by policy, marking absent: area=" + GetTag(oTargetArea) + ", anchor=" + GetTag(oPolicyFilteredAnchor)
            );
        }
        else
        {
            DL_LogNpc(oNPC, DL_DEBUG_BASIC, "anchor not found, marking absent in area=" + GetTag(oTargetArea));
        }
        DL_ApplyUnavailableInteractionState(oNPC, DL_DIR_ABSENT);
        return;
    }
    ...
    if (GetArea(oNPC) != GetArea(oPoint))
    {
        DL_LogNpc(oNPC, DL_DEBUG_BASIC, "cross-area jump to anchor: area=" + GetTag(GetArea(oPoint)) + ", anchor=" + GetTag(oPoint));
    }

    if (DL_ShouldInstantPlace(oNPC, oTargetArea, oPoint))
    {
