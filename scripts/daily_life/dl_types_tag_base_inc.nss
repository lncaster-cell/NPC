...
const string DL_L_NPC_BASE_TAG = "dl_npc_base_tag";
...
string DL_GetNpcBaseTag(object oNPC)
{
    return GetLocalString(oNPC, DL_L_NPC_BASE_TAG);
}

int DL_IsRuntimeBaseObjectType(int nObjType)
{
    return nObjType != OBJECT_TYPE_AREA && nObjType != OBJECT_TYPE_CREATURE;
}

object DL_FindNpcBaseByTag(object oNPC)
{
    string sBaseTag = DL_GetNpcBaseTag(oNPC);
    int nIndex = 0;
    object oBase;
    int nObjType;
    ...
    oBase = GetObjectByTag(sBaseTag, nIndex);
    while (GetIsObjectValid(oBase))
    {
        nObjType = GetObjectType(oBase);
        if (DL_IsRuntimeBaseObjectType(nObjType))
        {
...
object DL_GetNpcBase(object oNPC)
{
    ...
    return DL_FindNpcBaseByTag(oNPC);
}
