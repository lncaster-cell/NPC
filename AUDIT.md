...
### 1.3 Жёсткий лимит 100 NPC на область (enforced) может приводить к «тихой» деградации

**Симптом:**

`AL_RegisterNPC` теперь содержит явный guard `AL_MAX_NPCS = 100`, который ограничивает плотный массив `n0..n99`. При превышении лимита новые NPC не регистрируются.
...
- В `AL_RegisterNPC` добавлен явный runtime-cap `AL_MAX_NPCS = 100`.
- Регистрация разрешена только для валидных не-PC существ в валидной area.
- Удаление и компактизация реестра выполняются через `swap-remove`, сохраняя плотность индексов.

### Рекомендации

- Обрабатывать случаи переполнения реестра (лог/алерт в интеграционном слое), так как отказ регистрации происходит без исключений.
- Не писать напрямую в `al_npc_count`/`al_npc_<idx>` из внешних скриптов.
