...
## Стандарт принятия решений по NWScript

Перед любой runtime-правкой в `daily_life/*.nss` фиксируем короткий порядок:

1. Проверяем, есть ли встроенное решение в NWScript/NWN2 и в справке NWN Lexicon.
2. Используем штатный API/механику как базовый вариант (приоритет производительности и предсказуемости).
3. Только если штатного пути нет — добавляем тонкий адаптер с явным runtime-контрактом в документации.

Это правило обязательно для всех этапов разработки, чтобы не плодить ad-hoc-костыли и сохранять совместимость с существующим контуром Daily Life.
