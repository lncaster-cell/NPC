// Minimal archive-derived activity/animation reference layer for the first
// BLACKSMITH/GATE_POST/TRADER/DOMESTIC WORK+SLEEP vertical slice.
// Source: legacy ambientlivev2 `scripts/ambient_life/al_acts_inc.nss`.
//
// Activity IDs below are intentionally limited to IDs that are actually
// applied by DL_ApplyArchiveActivityPresentation (directly for sleep and
// via DL_TryApplyWorkActivityPresentation for work directives).
const int DL_ARCH_ACT_NPC_SLEEP_BED = 5;
const int DL_ARCH_ACT_NPC_FORGE = 15;
const int DL_ARCH_ACT_NPC_FORGE_MULTI = 30;
const int DL_ARCH_ACT_NPC_MERCHANT_MULTI = 38;
const int DL_ARCH_ACT_NPC_GUARD = 43;

const string DL_ARCH_ANIMS_FORGE = "forge01, forge02, dustoff";
const string DL_ARCH_ANIMS_CRAFT = "craft01, gettable, dustoff";
const string DL_ARCH_ANIMS_DOMESTIC = "cooking01, cooking02, craft01, dustoff, gettable, getground";
const string DL_ARCH_ANIMS_TRADE = "bored, getground, gettable, openlock, sleightofhand, yawn";
const string DL_ARCH_ANIMS_GUARD = "bored, lookleft, lookright, sigh";
const string DL_ARCH_ANIMS_SLEEP_BED = "laydownB, proneB";
