...
- Hardening 2026-04-21:
  - исправлен double-count observed population в wrap-фазе round-robin pass;
  - empty-registry bootstrap переведён в полный recovery-scan вместо частичного budget-slice;
  - area pass теперь opportunistic-регистрирует активных NPC, замеченных во время scan, чтобы registry быстрее сходился к фактической популяции.
...
- Runtime/log/diag контур:
  ...
  - `dl_chat_log` и `dl_chat_debug` снова управляют реальным `PrintString`-выводом вместо no-op заглушек.
...
4. `P2`: переход от count-only registry к полноценному dense area-registry, чтобы убрать зависимость worker hot-path от полного area scan.
5. `P2`: точечная оптимизация transition/lookup churn в hot-tier area.

## 4.1 Что синхронизировано в документации на 2026-04-21
...
- После аудита hot-path синхронизированы текущие исправления worker/registry:
  - устранён double-count `dl_area_pass_last_seen` в wrap-pass;
  - empty-registry recovery больше не оставляет area в частично зарегистрированном состоянии;
  - runtime/log/chat заглушки снова подчиняются module-флагам вместо полного отключения.
