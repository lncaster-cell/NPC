...
Идея этого файла: **сводка по runtime («что / почему / как / где / что дальше»), которая читается после `12_MASTER_PLAN` и вместе с профильными источниками**.
...
Дополнительная профильная детализация redesign Daily Life vNext вынесена в `docs/12B_DAILY_LIFE_VNEXT_CANON.md`. Этот документ не заменяет `12B`, а фиксирует отдельный нормативный слой новой модели повседневной жизни NPC.
...
Для legacy-обратной совместимости со старыми именами `al_*` используется централизованный mapping-док: `docs/12B_DAILY_LIFE_V1_LEGACY_TO_RUNTIME_MAPPING.md`.
