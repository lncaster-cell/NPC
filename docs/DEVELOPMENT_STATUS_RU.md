...
- Текущая стадия City Response:
  ...
  - ✅ shared-local contract dedupe: `dl_cr_last_guard`, `dl_cr_detain_dialog` и `dl_cr_investigate_*` переведены на единые символы city-response include (без дублирования в legal/crime include), что снижает риск дрейфа ключей между слоями.
