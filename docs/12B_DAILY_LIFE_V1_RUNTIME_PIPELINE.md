...
Система должна:
- работать только полноценно в `HOT-area`;
- использовать `WARM` как подготовительный слой;
...
`DL_OnAreaLeftByLastPlayer(area)`

Порядок действий:
1. area-controller оценивает соседние области и новый tier;
2. если area уходит в `WARM`, полная жизнь выключается, оставляется только подготовительный слой;
3. если area уходит в `FROZEN`, отключаются локальные активные jobs;
4. runtime state сохраняется как проекция, но не как полная историческая симуляция.
