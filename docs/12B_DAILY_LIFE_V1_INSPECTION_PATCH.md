...
1. `docs/12B_DAILY_LIFE_V1_INSPECTION_PATCH.md`
...
Примеры:
- `DLV1_ResolveDirective(...)`
- `DLV1_RunResync(...)`
- `DLV1_AreaWorkerTick(...)`
- `DLV1_RequestFunctionSlotReview(...)`

Нормативное правило:
- обозначения вида `DL_...` в runtime pipeline и data contracts трактовать как концептуальный уровень;
- в реальном NWScript implementation API использовать только `DLV1_...`.
...
- единый префикс implementation API = `DLV1_`;
