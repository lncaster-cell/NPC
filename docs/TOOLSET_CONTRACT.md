...
Stage I.1 добавляет отдельный bounded `OnDisturbed` runtime слой как inventory/theft foundation (без crime/alarm escalation).
...
| `al_react_type` | int | Stage I.1 классификация disturbance (`1=added`, `2=removed`, `3=stolen`). |
| `al_react_stage` | int | Stage I.1 служебный stage marker (`1=disturbed foundation`). |
| `al_react_resume_flag` | int (0/1) | Stage I.1 флаг попытки resume после локальной реакции. |
| `al_react_last_source` | object/string | Stage I.1 последний source из `GetLastDisturbed()`. |
| `al_react_last_item` | object/string | Stage I.1 последний item из `GetInventoryDisturbItem()`. |
...
- Stage I.1 реализует только inventory/theft disturbance foundation (`OnDisturbed`) с bounded local override/resume.
- `OnBlocked` остаётся отдельным Stage I.0 local navigation helper.
- Crime/alarm escalation не входит в текущий этап и переносится на следующий PR (Stage I.2).
