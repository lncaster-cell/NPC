...
Daily Life получает:
- текущий режим городской реакции;
- допустимые city-level directives и ограничения по area;
- сигналы тревоги, укрытия, duty-переназначения.
...
Нельзя:
- подменять Daily Life контуром тревоги;
- смешивать routine pipeline и city alarm FSM в одну state machine;
- считать тревогу нормальным базовым режимом Daily Life.
...
5. учитывается локальный state area tier и внешние overrides;
