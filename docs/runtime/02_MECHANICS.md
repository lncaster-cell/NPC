...
- named NPC не респаунятся; восполняется только дефицит unnamed населения.
- respawn и materialization разделены (новый объект vs возврат существующего).

Подробная спецификация и контракты: `docs/runtime/10_NPC_RESPAWN_MECHANICS.md`.
