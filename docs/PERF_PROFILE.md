...
6. Для PR с изменениями в `scripts/ambient_life/al_*` CI-check `Ambient Life Perf Gate` завершился успешно (`Perf gate passed`).
...
Используйте раздел **«5) Gate в TASKS / PR-review»** этого документа как обязательный pre-review checklist.
