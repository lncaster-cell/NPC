...
8. Новые runtime-логи добавлять только через доменные обёртки (`DL_LogTransitionEvent`, `DL_LogSocialEvent`, `DL_LogCrimeEvent`) в формате `domain/event_code/npc_tag/key_context`; прямые вызовы базового logger запрещены.
