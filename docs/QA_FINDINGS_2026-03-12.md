...
**Статус.** 🟡 Частично выполнено / остаётся открытым: операторские правила есть, но in-repo инструмент `scripts/ambient_life/al_link_preflight.py` отсутствует, а подтверждённый автоматический gate для linked-графа не зафиксирован.

**Источник исполнения (на текущий момент).** Только ручная операторская проверка по `docs/LINKED_GRAPH_OPERATIONS.md` и PR-checklist; отдельный CI job / внешний репозиторий / внутренний pipeline для linked preflight не задокументирован.
...
2. Зафиксировать реальный источник автоматического запуска linked preflight (имя CI job или ссылка на внешний/internal pipeline) и синхронизировать его во всех runbook/checklist документах.
3. Уточнить пороги `WARN/ERROR` для degree/cluster-size по итогам perf-замеров S80/S100/S120.

**Критерий закрытия.** Риск считается закрытым только когда в PR/CI приложен артефакт linked preflight (JSON/text-отчёт) и есть ссылка на зелёный запуск конкретной джобы/pipeline, где этот отчёт сформирован.
