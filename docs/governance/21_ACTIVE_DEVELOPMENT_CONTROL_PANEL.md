...
## 0) Быстрый статус на 2026-04-08 (перезапуск Daily Life с нуля)

Ниже — короткая operational-сводка, чтобы не гадать «где мы сейчас».

### 0.1 Что уже сделано
- Legacy-реализация `Daily Life v1` архивирована в `archive/daily_life_v1_legacy/scripts/daily_life/`.
- Активный `scripts/daily_life/` очищен под перепись v2 с нуля.
- Создан отдельный v2-протокол пошаговой реализации: `docs/runtime/40_DAILY_LIFE_V2_REWRITE_PROGRAM_RU.md`.

### 0.2 Что проверяем прямо сейчас
1. Корректность проектирования v2-контракта (данные, события, ограничения).
2. Чистоту репозитория после архивирования v1 (нет активных ссылок на старый runtime как на production-код).
3. Готовность к итерациям «одна функция → одна проверка».

### 0.3 Что сейчас ещё НЕ заявляется как «готово»
- Нет рабочего runtime v2 (только подготовка).
- Нет v2 acceptance/runbook.
- Нет owner-run для v2.
...
### Текущая фаза
- **Phase:** active development execution (Rewrite reset)
- **Главная цель фазы:** чисто переписать Daily Life-контур с нуля по контролируемому протоколу.
- **Текущий implementation target:** `Daily Life v2 — rewrite from scratch`
...
- **Цель:** зафиксировать v2 design baseline и репозиторий без технических хвостов v1 в активной зоне.
- **Критерий выполнения:** готовый v2 rewrite-план + архивирование v1 + чистый стартовый workspace.
...
1. `README.md`
2. `docs/canon/17_UNIFIED_GAME_DESIGN_BRIEF_RU.md`
3. `docs/entry/12_MASTER_PLAN.md`

### Для ежедневной активной работы
1. `docs/governance/21_ACTIVE_DEVELOPMENT_CONTROL_PANEL.md` ← этот файл
2. `docs/runtime/40_DAILY_LIFE_V2_REWRITE_PROGRAM_RU.md`
3. `docs/runtime/12B_DAILY_LIFE_V1_IMPLEMENTATION_STATE.md` (как retrospective source)
4. `archive/daily_life_v1_legacy/scripts/daily_life/` (как технический reference)
...
- `docs/canon/17_UNIFIED_GAME_DESIGN_BRIEF_RU.md` — общий канон проекта
- `docs/canon/12A_WORLD_MODEL_CANON.md` — world/legal канон
- `docs/runtime/12B_RUNTIME_MASTER_PLAN.md` — runtime и daily-life/runtime границы
- `docs/canon/12C_PLAYER_PROPERTY_SYSTEM.md` — property
- `docs/canon/12D_WORLD_TRAVEL_CANON.md` — travel
- `docs/canon/12E_TRADE_AND_CITY_STATE_CANON.md` — trade/city-state
- `docs/canon/13_AGING_AND_CLAN_SUCCESSION.md` — aging/succession
- `docs/canon/14_CLAN_SYSTEM_DESIGN.md` — clans
...
- `docs/audits/30_AUDIT_AND_INSPECTION_INDEX.md` (единый вход в датированные audit/inspection отчёты и текущую операционную сверку doc↔code)
...
- `docs/entry/01_PROJECT_OVERVIEW.md`
- `docs/research/legal_system_design.md` (redirect-only)
...
- поддержание актуального runbook для scripted smoke цикла (`docs/runtime/12B_DAILY_LIFE_V1_SMOKE_RUNBOOK.md`).
...
- запись результата в `docs/runtime/12B_DAILY_LIFE_V1_ACCEPTANCE_JOURNAL.md`.
