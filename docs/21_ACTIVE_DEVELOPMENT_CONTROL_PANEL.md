...
| O-03 | Граница Daily Life vs City Response | Мягкие отклонения в DL, серьёзный инцидент -> City Response | clarification_required | нужно отдельное пояснение и подтверждение владельцем |
...
4. `O-03` — **остался открытым**: требуется отдельное короткое решение по границе Daily Life vs City Response.

До решения `O-03` разрешено использовать временное рабочее правило:
- Daily Life обрабатывает локальные отклонения и short-lived disturbances;
- подтверждённый высокий риск (alarm/пожар/массовое нарушение) эскалируется в City Response.
