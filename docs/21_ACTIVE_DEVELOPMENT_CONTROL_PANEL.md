...
### Таблица решений (заполняется владельцем)

| ID | Вопрос | Рекомендуемый вариант | Статус | Текущее значение |
|---|---|---|---|---|
| O-01 | Первый playable slice | `LAW/CRAFT/TRADE_SERVICE` + `SLEEP/WORK/SERVICE/DUTY` | pending | — |
| O-02 | Базовый override для smoke | `QUARANTINE` (primary), `FIRE` (secondary) | pending | — |
| O-03 | Граница Daily Life vs City Response | Мягкие отклонения в DL, серьёзный инцидент -> City Response | pending | — |
| O-04 | Формат подтверждения готовности | scripted smoke + expected-state журнал | pending | — |
...
## 12) Прямые вопросы к владельцу (нужно ответить перед следующим implementation-pass)

1. Подтверждаешь ли ты `O-01` без расширения slice до конца Milestone A?
2. Подтверждаешь ли ты `O-02` с `QUARANTINE` как primary smoke override?
3. Подтверждаешь ли ты `O-04` (scripted smoke + expected-state журнал) как обязательный формат проверки?

Если ответы «да», следующий проход можно делать без остановок на продуктовые развилки.
