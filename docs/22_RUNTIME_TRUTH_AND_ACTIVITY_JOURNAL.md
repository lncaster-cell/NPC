...
- Внутренняя санитарная чистка идёт по runtime-first принципу: сначала оптимизируются hot-path schedule/resolver/worker/resync/materialize/anchor helper-функции, потом менее частый legacy include-layer.
