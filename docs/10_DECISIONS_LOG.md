...
### DEC-2026-03-17-006: Heartbeat как управляемый вспомогательный канал, а не фоновый контур
- **Дата:** 2026-03-17
- **Статус:** accepted
- **Контекст:** Требовалось зафиксировать роль heartbeat в runtime так, чтобы не нарушать базовую архитектуру `event-driven + bounded + area-centric` и не деградировать масштабируемость за счёт «пустых» фоновых вызовов.
- **Решение:** Heartbeat не входит в базовый каркас исполнения и разрешён только как управляемый auxiliary-канал через отдельный контроллер (Heartbeat Gate / Supervisor). Политика: включать heartbeat только по явным условиям, выполнять только лёгкие задачи и гарантированно отключать (`SetEventHandler(..., "")`/снятие script handler), когда условия исчезают.
- **Обоснование:** Подход «heartbeat везде, а внутри ранний выход» остаётся дорогим из-за самого факта вызова обработчиков в большом количестве областей/объектов. Управляемое включение/отключение сохраняет производительность и предсказуемость runtime.
- **Последствия:** Любые новые механики, требующие периодики, сначала должны пройти через heartbeat-политику и доказать, что event-driven путь недостаточен; постоянный heartbeat-фон считается анти-паттерном.
- **Ссылки:** `docs/12B_RUNTIME_MASTER_PLAN.md`, `docs/06_SYSTEM_INVARIANTS.md`, `docs/19_AI_RESEARCH_ENGINE_CONSTRAINTS_RU.md`
