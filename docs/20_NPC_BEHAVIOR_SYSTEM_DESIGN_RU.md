...
`время движка + профиль NPC + базовый контекст места + внешний incident context / override → текущая директива → локальное исполнение`
...
### 16.5 Unified incident contract

Внешние городские состояния должны описываться единым incident-layer, а не набором частных механик под каждый новый сценарий.

Минимальный контракт:
- `incident_type = none / fire / quarantine / alarm / curfew / riot / ...`;
- `incident_stage`;
- `incident_severity`;
- `incident_scope = city / district / area-set`;
- `affected_roles`;
- `restricted_area / danger_area / blocked_routes / safe_point / panic_point / incident_anchor`.

Поведенческий контракт для NPC:
- базовый schedule/profile остаётся источником мирной жизни;
- внешний incident context может временно включать role-based override;
- переход выполняется через общий pipeline `interrupt -> temporary behavior -> resync/resume`;
- переключение идёт по событиям `incident_start / incident_stage_changed / incident_end / incident_resync_requested`, а не через постоянные проверки каждым NPC.
...
### R5 — Unified incident overrides
- единый incident context вместо отдельных fire/quarantine subsystems;
- City Response overrides;
- role-based temporary behavior profiles;
- event-driven interrupt / resync / resume contract;
- Legal flags integration;
- Property access constraints;
- Trade/City context modifiers;
- area-level incident metadata.
...
`Ambient Life Daily Life vNext = engine time + NPC profile + base/building context + unified incident context + directive resolver + materialization + bounded active-area execution + population-aware replacement`

Обновлённая короткая формула:

`Daily Life vNext = time-driven directives + external incident context + role-based override + place-aware materialization + bounded HOT-area execution + neighbor warm-up + zero-idle FROZEN policy`
