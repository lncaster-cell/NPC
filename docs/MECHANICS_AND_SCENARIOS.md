...
## 15. Механика очистки city enemy-реестра и возврата alarm в `IDLE`

1. Городской enemy-реестр хранит не только признак врага, но и `al_city_enemy_last_active_tick` (последний тик активности врага в городе).
2. При повторной регистрации того же врага таймштамп активности обновляется, что отделяет «временного агрессора» от «долговременного врага».
3. Перед `AL_CityAlarmTryClear` выполняется обслуживание `AL_CityRegistryPruneEnemies`.
4. `AL_CityRegistryPruneEnemies` удаляет записи, если:
   - объект больше невалиден или не является существом;
   - объект вышел из города (city id area объекта не совпадает с city id реестра);
   - объект неактивен дольше окна `AL_CITY_ENEMY_INACTIVE_TICK_WINDOW`.
5. При удалении из реестра (включая timeout/invalid-state) обязательно очищаются флаги объекта:
   - `al_city_enemy_active`;
   - `al_city_enemy_city_id`;
   - `al_city_enemy_last_active_tick`.
6. Тревога города возвращается в `IDLE` только после последовательности:
   - `AL_CityRegistryPruneEnemies` не оставил активных врагов,
   - `AL_CityAlarmTryClear` перевёл город в `CLEARING` и затем в `RECOVERY`,
   - runtime-тик завершил recovery-переход и применил `AL_CITY_ALARM_IDLE`.

### Сценарии
- Сценарий A: агрессор погиб/исчез — запись удаляется, флаги врага очищаются, город доходит до `IDLE` после recovery.
- Сценарий B: агрессор покинул город (другая area/city) — запись вычищается в prune, ложный «вечный alarm» не сохраняется.
- Сценарий C: агрессор давно не проявлял активность — по таймауту удаляется из enemy-реестра, после чего возможен штатный clear alarm.
