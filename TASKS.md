...
- Linked-graph QA-gate перед rollout linked-area правок: обязательный запуск `python3 scripts/ambient_life/al_link_preflight.py --input <areas.json> --format json` (или `--format text`); при `status=ERROR`/exit code `1` rollout блокируется до исправления (`exit code 2` — фатальная ошибка входных данных/формата).
